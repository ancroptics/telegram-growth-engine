...
async def get_broadcast_by_id(bc_id: int) -> Optional[dict]:
    return await Database.fetch_one("SELECT * FROM broadcasts WHERE id = $1", bc_id)
...
async def get_global_stats() -> dict:
    users = await Database.fetch_one("SELECT COUNT(*) as count FROM end_users")
    channels = await Database.fetch_one("SELECT COUNT(*) as count FROM managed_channels WHERE is_active = TRUE")
    owners = await Database.fetch_one("SELECT COUNT(*) as count FROM channel_owners")
    today_stats = await Database.fetch_one(
